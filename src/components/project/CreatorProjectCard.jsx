  ...
  return (
    <div className="bg-[#111116] border border-white/[0.08] hover:border-white/[0.16] rounded-2xl p-4 sm:p-5 transition-all duration-300 flex flex-col sm:flex-row gap-4 sm:gap-5 group shadow-lg hover:shadow-[0_12px_36px_rgba(0,0,0,0.5)]">
      ...
      <div className="w-24 sm:w-32 md:w-36 shrink-0 aspect-[2/3] rounded-xl overflow-hidden border border-white/10 bg-[#1A1A22] relative group/poster self-start">
        <img
          src={thumbnail || '/images/hero-bg.png'}
          alt={title}
      ...
      <div className="flex-1 min-w-0 flex flex-col justify-between py-0.5">
        <div>
          ...
          <div className="flex items-start justify-between gap-3 mb-2">
            <div className="min-w-0 flex-1">
              <h3 className="font-['Fraunces',_serif] text-lg sm:text-xl font-semibold text-white tracking-[-0.01em] break-words group-hover:text-purple-light transition-colors leading-snug">
                {title}
              </h3>
              {metaParts.length > 0 && (
            ...
            <div className="flex flex-col items-end gap-1.5 shrink-0">
              <span className={`px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded-full border ${STATUS_BADGES[status] || 'border-white/20 text-white/60'}`}>
                {status}
              </span>
              {pendingApplications > 0 && (
                <span className="px-2 py-0.5 text-[10px] font-bold text-purple-light bg-purple/15 border border-purple/30 rounded-full flex items-center gap-1 animate-pulse">
              ...
              <span className="font-medium text-white/80">
                {totalRequired > 0 ? (
                  <>
                    ...
                    <span className="text-white/40 font-normal">positions filled</span>
                  </>
                ) : (
                  <span className="text-white/40 italic">No roles added yet</span>
                )}
              </span>
              {totalRequired > 0 && (
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 text-xs pt-2 text-white/50">
            {pendingApplications > 0 ? (
              ...
              <span className="text-white/35">No pending applications</span>
            )}
        ...
        {/* Action Row: Single Primary CTA */}
        <div className="pt-4 mt-2 border-t border-white/[0.06] flex items-center justify-end">
          <Link
            to={`/project/${id}`}
            id={`manage-project-${id}-btn`}
            className="inline-flex items-center gap-2 px-4 py-2 bg-purple text-white text-xs font-semibold rounded-xl transition-all duration-200 hover:bg-purple-dark hover:shadow-[0_0_18px_rgba(98,57,191,0.35)] active:scale-[0.98]"
          >
