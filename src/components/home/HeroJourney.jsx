...
export default function HeroJourney() {
  ...
  return (
    <div
      className="relative w-full max-w-[420px] flex flex-col items-center justify-center select-none outline-none focus-visible:ring-2 focus-visible:ring-[#6239BF] focus-visible:ring-offset-4 focus-visible:ring-offset-[#0A0A0F] rounded-2xl p-1"
      tabIndex={0}
    ...
    >
      ...
      <div className="w-full flex items-center justify-between mb-3 sm:mb-4 px-1">
        <span className="text-[8px] sm:text-[9px] font-bold tracking-[0.18em] uppercase text-white/40">
          From Idea to Credit
        </span>
        <span className="text-[9px] sm:text-[10px] font-semibold text-white/55 tabular-nums">
          {String(step + 1).padStart(2, '0')} / 04
        </span>
      </div>

      {/* ── Progress Steps ── */}
      <div className="flex items-center gap-1 mb-3 sm:mb-4 w-full">
        {STEPS.map((s, i) => (
          <button
            key={s.key}
            onClick={() => go(i)}
          ...
          >
            <div className={`w-full h-[2.5px] rounded-full transition-all duration-400 ${
              i <= step ? 'bg-[#6239BF]' : 'bg-white/10'
            } ${i === step ? 'shadow-[0_0_8px_rgba(98,57,191,0.4)]' : ''}`} />
            <span className={`text-[7.5px] sm:text-[8px] tracking-wider uppercase font-semibold transition-colors duration-300 ${
      ...
      <div className="w-full relative h-[61px] sm:h-[66px] mb-2.5 sm:mb-3">
        <AnimatePresence mode="wait">
          ...
          >
            <p className="font-['Bebas_Neue',_sans-serif] text-[19px] sm:text-2xl font-normal text-white mb-1 leading-none tracking-wide">
              {STEPS[step].heading}
            </p>
            <p className="text-[10px] sm:text-[10.5px] text-white/50 leading-relaxed max-w-[288px] mx-auto">
              {STEPS[step].sub}
            </p>
          </motion.div>
        </AnimatePresence>
      ...
      {/* ── Visual Stage & Side Navigation Wrapper ── */}
      <div className="relative w-full">
        ...
        <button
          ...
          className={`absolute -left-7 sm:-left-8 lg:-left-16 top-1/2 -translate-y-1/2 z-30 w-[52px] h-[52px] sm:w-[58px] sm:h-[58px] rounded-full border flex items-center justify-center transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#6239BF] focus-visible:ring-offset-2 focus-visible:ring-offset-[#0A0A0F] ${
            step === 0
        ...
        <div className="relative w-full h-[224px] sm:h-[232px]">
          ...
          <div
            className={`absolute top-[53%] left-1/2 -translate-x-1/2 -translate-y-1/2 z-10 rounded-xl border p-2.5 sm:p-3 w-[140px] sm:w-[148px] transition-colors duration-400 ${
              isCompleted
          ...
          >
            ...
            <div className="flex items-center justify-between mb-2">
              <span
                className={`inline-flex items-center gap-1 px-1.5 py-0.5 text-[7.5px] sm:text-[8px] font-bold tracking-wider rounded-full border ${
                  isCompleted
                    ? 'border-emerald-500/30 text-emerald-400 bg-emerald-500/10'
                    : 'border-[#6239BF]/35 text-purple-light bg-[#6239BF]/15'
                }`}
              >
                <span className={`w-1 h-1 rounded-full ${isCompleted ? 'bg-emerald-400' : 'bg-[#6239BF] animate-pulse'}`} />
                {isCompleted ? 'COMPLETED' : 'OPEN'}
              </span>
              <span className="text-[7.5px] text-white/30 font-medium">Drama</span>
            </div>

            {/* Title */}
            <p className="font-['Bebas_Neue',_sans-serif] text-[14.5px] sm:text-base font-normal text-white leading-tight mb-0.5 tracking-wide">
              AFTERLIGHT
            </p>
            <p className="text-[8px] text-white/40 mb-2">Pune</p>
            ...
            <AnimatePresence mode="wait">
              {isCompleted ? (
                ...
                >
                  <p className="text-[7.5px] font-bold tracking-[0.12em] uppercase text-emerald-400/80 mb-0.5">Your Credit</p>
                  <p className="text-[10px] font-semibold text-white">Cinematographer</p>
                </motion.div>
              ) : (
                <motion.div
                  key="roles"
                  ...
                  className="border-t border-white/[0.06] pt-1.5"
                >
                  <p className="text-[7.5px] font-bold tracking-[0.12em] uppercase text-white/35 mb-1">
                    {showAvatars ? `${ROLES.filter((_, i) => i < 2).length} of ${ROLES.length} joined` : `${ROLES.length} roles needed`}
                  </p>
                  <div className="flex flex-wrap gap-0.5">
                    {ROLES.map((r) => (
                      <span key={r.name} className="text-[8px] text-white/50 px-1 py-0.5 rounded border border-white/[0.08] bg-white/[0.02]">
                        {r.name}
                      </span>
                    ))}
                  </div>
          ...
          <AnimatePresence>
            {showRoles && ROLES.map((role, i) => {
              ...
              const positions = [
                'absolute top-1.5 sm:top-2.5 right-2 sm:right-5 z-20',        // Actor: upper-right
              ...
              return (
                ...
                >
                  <div className={`rounded-lg border px-2 sm:px-2.5 py-1 sm:py-1.5 text-center transition-colors duration-400 min-w-[72px] sm:min-w-[80px] ${
                    accepted
                    ...
                    {showAvatars && (
                      ...
                      >
                        {accepted ? (
                          <>
                            <div className="w-4 h-4 rounded-full bg-[#6239BF]/20 border border-[#6239BF]/30 flex items-center justify-center">
                              <span className="text-[6.5px] font-bold text-purple-light">{role.initials}</span>
                            </div>
                          ...
                          <div className="w-4 h-4 rounded-full border border-dashed border-white/20 flex items-center justify-center">
                            <span className="text-[6.5px] text-white/30">?</span>
                          </div>
                        )}
                      </motion.div>
                    )}
                    <p className="text-[8px] sm:text-[9px] font-semibold text-white/80 leading-tight">{role.name}</p>
                    <p className={`text-[6.5px] sm:text-[7.5px] font-semibold tracking-wider uppercase mt-0.5 ${
          ...
          <AnimatePresence>
            {isCompleted && (
              <motion.p
                ...
                exit={{ opacity: 0 }}
                transition={quick(reduced)}
                className="absolute bottom-1.5 sm:bottom-2 left-1/2 -translate-x-1/2 whitespace-nowrap text-[8px] sm:text-[9px] text-emerald-400/70 font-medium text-center z-10"
              >
      ...
      <div className="flex items-center justify-center w-full mt-3 sm:mt-4 px-1">
          ...
          {STEPS.map((_, i) => (
            <div
              ...
              className={`rounded-full transition-all duration-300 ${
                i === step ? 'w-4 h-1 bg-[#6239BF]' : 'w-1 h-1 bg-white/20'
              }`}
            />
          ))}
        </div>
